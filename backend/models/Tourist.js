...
export async function registerTourist(tourist) {
  ...
  const result = await db.run(
    `INSERT INTO users (name,email,password,phone,publicKey,privateKey,aadhaarPassport,itinerary,tripStartDate,tripEndDate,emergencyContacts,kycImagePath,digitalIdJson,digitalIdHash) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      tourist.name,
      tourist.email,
  ...
  return { id: result.lastID, name: tourist.name, email: tourist.email };
}

export async function loginTourist(email) {
  const db = await dbPromise;
