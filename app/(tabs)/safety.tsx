...
export default function SafetyScreen() {
  ...
  const { currentLocation, safetyScore = 0 } = useLocation();
  const [refreshing, setRefreshing] = useState(false);
  const [currentZone, setCurrentZone] = useState<any>(null);

  const contacts = tourist?.emergencyContacts ?? [];
  ...
  const getZoneStatus = () => {
    if (!currentZone) {
      return {
        ...
        message: t('safety.inSafeArea') || 'You are in a safe area',
      };
    }
    ...
    return {
      ...
      title: currentZone.riskLevel === 'high' ? t('safety.dangerZone') : t('safety.cautionZone'),
      message: `${t('safety.exitZone') || 'Exit the zone!'}\nZone: ${currentZone.name || 'N/A'}`,
    };
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.statusCard}>
        ...
        {zoneStatus.message && <Text style={styles.zoneMessage}>{zoneStatus.message}</Text>}
      </View>
      ...
      <View style={styles.statusCard}>
        ...
        <View style={styles.monitoringStatus}>
          ...
          <Text style={styles.monitoringText}>{t('safety.monitoringActive') || 'Activity monitoring active'}</Text>
        </View>
        <Text style={styles.monitoringDescription}>
          {t('safety.monitoringDesc') ||
      ...
      {contacts.length > 0 && (
        <View style={styles.emergencyCard}>
          <Text style={styles.emergencyTitle}>{t('safety.emergencyContacts')}</Text>
